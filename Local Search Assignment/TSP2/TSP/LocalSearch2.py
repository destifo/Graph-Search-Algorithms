...
class LocalSearch:
    ...
    def __init__(self, edgefile) -> None:
        self.graph = Graph()
        self.__build_graph(edgefile)


    def __build_graph(self, edgefile):
        fh = open(edgefile)
        for line in fh:
            line = line.split(',')
            self.graph.addNode(line[0])
            self.graph.addNode(line[1])
    ...
    def GA_search(self, init_popn_size=10, generations=70):
        n = init_popn_size
        popn = []
        best_sol = None
        best_val = float('inf')
